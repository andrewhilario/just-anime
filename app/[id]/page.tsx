...
import { AnimeProvider, MangaProvider } from "@/lib/constants";
...
const AnimeInfo = (props: Props) => {
  ...
  const [visibleEpisodes, setVisibleEpisodes] = useState(30);
  const [prov, setProv] = useState(AnimeProvider.ANIMEFOX);
  ...
  useEffect(() => {}, [animeInfo, query]);

  const handleLoadMore = () => {
    setVisibleEpisodes((prevVisibleEpisodes) => prevVisibleEpisodes + 30);
  };
  ...
  return (
    <div className="w-full">
      <Navbar />
      {(query.get("type") !== "manga" || !query.get("type")) && (
        <>
          <div className="px-5 lg:px-20 mx-auto grid grid-cols-1 lg:grid-cols-4 lg:space-x-4 space-y-4 items-center mt-10">
              ...
              {animeInfo?.episodes?.map((episode: any) => {
                return (
                  <div
                    ...
                    onClick={() => {
                      router.push(
                        `/${animeInfo.id}/watch?episode=${episode.id}`
                      );
                    }}
                  >
